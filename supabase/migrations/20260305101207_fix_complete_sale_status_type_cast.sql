/*
  # Fix Complete Sale Status Type Casting

  ## Overview
  Fixes the type casting issue where the status column expects sale_status enum
  but receives text. This migration updates the complete_sale function to properly
  cast the status parameter.

  ## Changes
  1. Drop the existing complete_sale function
  2. Recreate it with proper type casting for the status field
  
  ## Technical Details
  - Explicitly cast status to sale_status enum type
...
BEGIN
  ...
  -- Insert sale record with proper type casting
  INSERT INTO sales (
    sale_number,
    location_id,
    customer_id,
