/*
  # Fix Negative Inventory and Add Constraints

  ## Overview
  This migration fixes existing negative inventory records and adds constraints
  to prevent future data integrity issues.

  ## Changes Made

  ### 1. Clean Up Duplicate/Negative Inventory Records
  - Consolidate duplicate inventory records for the same product/location
  - Reset negative quantities to 0 as a safety measure
  - This is a data correction step before adding constraints

  ### 2. Add Stock Validation Constraints
  - Prevent negative inventory quantities
  - Ensure payment validation for completed sales

  ### 3. Improve complete_sale RPC Function
  - Add row-level locking to prevent race conditions
  - Add stock validation before processing
  - Better error messages

  ## Important Notes
  - Negative inventory indicates past overselling issues
  - After this migration, overselling will be prevented
  - Manual stock adjustments may be needed to reflect true inventory
*/

-- Step 1: Identify and delete all but one record for duplicates
-- Keep the first created record for each product/location combo
WITH ranked_inventory AS (
  SELECT 
    id,
    ROW_NUMBER() OVER (
      PARTITION BY location_id, product_id, COALESCE(variant_id, '00000000-0000-0000-0000-000000000000'::uuid)
...
-- Step 2: Sum up quantities from stock_movements to calculate correct inventory
-- This creates a temporary fix by resetting to 0 (manual correction needed)
UPDATE inventory 
SET quantity = 0
WHERE quantity < 0;

-- Step 3: Add constraint to prevent negative inventory
...
-- Step 4: Add payment validation constraint
ALTER TABLE sales
DROP CONSTRAINT IF EXISTS sales_payment_valid;

ALTER TABLE sales
ADD CONSTRAINT sales_payment_valid
CHECK (
  status != 'completed' OR 
  (amount_paid >= total_amount AND amount_paid IS NOT NULL AND total_amount IS NOT NULL)
);

-- Step 5: Add unique constraint to prevent duplicate inventory records
CREATE UNIQUE INDEX IF NOT EXISTS inventory_unique_location_product_variant
ON inventory(location_id, product_id, COALESCE(variant_id, '00000000-0000-0000-0000-000000000000'::uuid));

-- Step 6: Replace complete_sale function with improved version
